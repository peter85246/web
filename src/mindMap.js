...
function createLine(from, to, side) {
  ...
  window.addEventListener("resize", function () {
    ...
    resizeTimeout = setTimeout(updateMindMapLayout, 100);
  });

  // 更新整个心智图的布局
  function updateMindMapLayout() {
    const container = document.querySelector(".content-box-middle-alarm");
    const containerRect = container.getBoundingClientRect();
    const centralNode = document.getElementById("central-node");

    // 计算缩放比例，确保中心节点始终在容器中心
    const scale = Math.min(
      containerRect.width / (centralNode.offsetWidth + 400), // 预留空间以适应分支节点
      containerRect.height / (centralNode.offsetHeight + 400) // 预留空间以适应分支节点
    );

    // 设置中心节点的样式
    centralNode.style.transform = `translate(-50%, -50%) scale(${scale})`;
    centralNode.style.position = "absolute";
    centralNode.style.left = "50%";
    centralNode.style.top = "50%";

    // 更新分支节点和连线
    updateNodesAndLines(scale);
  }

  // 更新所有连接线和节点的位置
  function updateNodesAndLines(scale, centralNode) {
    const centralRect = centralNode.getBoundingClientRect();
    // 计算中心节点中心点坐标
    const centralX = centralRect.left + centralRect.width / 2;
    const centralY = centralRect.top + centralRect.height / 2;

    document.querySelectorAll(".node:not(.central-node)").forEach((node) => {
      ...
      // 调整节点的位置
      node.style.transform = `translate(${newPosition.x}px, ${newPosition.y}px)`;

      // 更新连线
      createLine(parent, node, side, scale);
    });
  }
  ...
  function calculateNodePosition(
    ...
    // 为了保证节点不超出容器，需要考虑缩放比例
    const x = (side === "left" ? -1 : 1) * horizontalSpacing * level * scale;
    const y = verticalSpacing * (level - 1) * scale;

    // 根据中心节点位置计算分支节点的新位置
    const posX = centralX + x - (node.offsetWidth * scale) / 2;
    const posY = centralY + y - (node.offsetHeight * scale) / 2;
  ...
  document.addEventListener("DOMContentLoaded", function () {
    updateMindMapLayout(); // 首次加载时调用以设置初始布局
  });
...
function updateChildrenPositions(parentId, level, side, spacing, childIds) {
  ...
  childIds.forEach((id, index) => {
    ...
    if (side === "left") {
      child.style.left = `${
        parentRect.left - child.offsetWidth - 70
      }px`; /* 欄位水平間距*/
    } else {
      child.style.left = `${parentRect.right + 70}px`;
    }
